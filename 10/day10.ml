...
let angle_between_points p_base p_other =
  ...
  Float.atan2 (y2 - y1 |> Float.of_int) (x2 - x1 |> Float.of_int)
...
module Asteroid_map = struct
  ...
  type slopes = (float, Float_approx.comparator_witness) Set.t
  ...
  let parse_map map : t =
    ...
    let build_line p1 (lines : slopes) p2 =
      if Point.compare p1 p2 = 0 then lines
        ...
        let slope = angle_between_points p1 p2 in

        let new_lines = Set.add lines slope in
        (* if fst p1 = 1 && snd p1 = 2 then *)
...
let solve () =
  ...
  let _, best_observable = best_observatory map in
